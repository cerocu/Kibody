...
public class selectModo : MonoBehaviour {
			...
			if(manager && manager.IsInitialized())
			{
				...
				if(manager.IsUserDetected())
				{
					...
					if(manager.IsJointTracked(userId, iJointIndex))
					{
						...
						if (scaleX >= 0.2328125 && scaleX <= 0.278125) {
							if (scaleY <= 0.60625 && scaleY >= 0.3895833) {
								//Debug.Log (" X "+scaleX+" Y:"+scaleY);
								if(bandera == false){
									bandera = true;
									DatoInicio.dato.setSeleccion("cabeza");
									Application.LoadLevel ("SelecionarAvatar");
								}
							...
							if (scaleX >= 0.7453125  && scaleX <= 0.8) {
								if (scaleY <= 0.6083333 && scaleY >= 0.406225) {
								//Debug.Log (" X "+scaleX+" Y:"+scaleY);
								if(bandera == false){
									bandera = true;
									DatoInicio.dato.setSeleccion("cuerpo");
									Application.LoadLevel ("SelecionarAvatar");
								}
