...
//	public Vector3 BottomLeft;
	...
	void Start()
	{
		...
		Debug.Log ("posicion de la bolita");
		Debug.Log (OverlayObject.transform.position);
		if(OverlayObject)
		{
...
//						Vector3 vPosOverlay = backgroundImage.transform.TransformPoint(localPos);
						...
						if (bandera == true) {
										if (scaleX >= 0.696875 && scaleX <= 0.91875) {
											if (scaleY >= 0.47 && scaleY <= 0.65) {
									...
									if (scaleX <= 0.29 && scaleX >= 0.1125) {
										if (scaleY <= 0.65 && scaleY >= 0.47) {
											Debug.Log ("jugar");
